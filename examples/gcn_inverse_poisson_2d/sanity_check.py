...
I am getting the correct value for f_val
when we take the actual solution instead of the gcn output.
"""
import jax
...
def main(
    ...
    p_2d_actual = Poisson_2d(domain, u, f)
    p_2d_actual.get_K_f()
    p_2d_actual.get_u_fem()
    u_actual = p_2d_actual.u_unknown
...
# Gloabal stiffness matrix and consistent load vector
    ...
    Kf1f2 = [K_mat, f_force.reshape(-1,1), f_bound.reshape(-1,1), u_actual.reshape(-1,1)]
...
# Loss is typically a func of output and target
    def loss_fn(output, Kf1f2):
        K_mat, f_force, f_data, u = Kf1f2
        f_val = output[1]
        res = (K_mat @ u) / f_val - f_force + f_data / f_val
