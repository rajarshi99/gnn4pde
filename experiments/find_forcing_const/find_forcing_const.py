...
# Input is the xy coords at the degree of freedom
    ...
    data_node_inds = jax.random.permutation(data_node_selection_key, jnp.arange(dof_inds.shape[0]))[:num_internal_data_points]
...
# Other activations are tanh
    ...
    print("Internal data node xy coords", xy_inp[data_node_inds,:])
...
# Training of the GCN model
    ...
    iter_width_list = [int(num_iters/num_steps)] * num_steps
    lr_list = jnp.linspace(lr_init, lr_final, num_steps).tolist()
    step_id = 0
    tot_iters = 0
    for iter_width,lr in zip(iter_width_list,lr_list):
        print(f"{step_id} of {num_steps} fits")
        step_id += 1
        tot_iters += iter_width
        model = GCNModel(gcn, loss_fn, [rel_l2_err_fn, f_val_fn, penalty_fun])
        gcn, history = model.fit(u_gcn, A, deg, Kf1f2,
                                 learning_rate=lr,
                                 num_iters=iter_width,
                                 num_check_points=iter_width)
        history["lr"] = lr
        history_list.append(history)
        gcn_output = gcn(u_gcn, A, deg)
        u_gcn, f_curr = gcn_output
        u_gcn_full = p_2d.assemble_sol(u_gcn.reshape(-1))
        p_2d.plot_on_mesh(u_gcn_full,
                          f"Iter: {tot_iters}, f value: {f_curr}",
                          f"{output_dir}{num_points}_u_gcn_{step_id:03d}.png")
    ...
    iter_shift = 0
    for history,iter_width in zip(history_list,iter_width_list):
    ...
    metric_vals = jnp.array([ h["metric_vals"] for h in history_list])
    lr_vals = jnp.array([ h["lr"] for h in history_list])
    ...
    return iter_ids, loss_vals, metric_vals, ["rel_l2_err_fn", "f_val_fn", "penalty_fun"], init_time, train_time
